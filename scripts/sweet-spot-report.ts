...
for (const p of sweet.sort((a, b) => (wineGorillaScore(b) + b.wineQuality!) - (wineGorillaScore(a) + a.wineQuality!))) {
  console.log(`  ${p.name} — 🦍 ${wineGorillaScore(p)} · ⭐ ${p.wineQuality} (${p.wineQualitySource})${p.ontarioVQA ? " · VQA" : ""}`);
}

console.log(`\nDYNAMIC COMBINED TOP 10:`);
rated
  .map((p) => ({ p, g: wineGorillaScore(p), c: (wineGorillaScore(p) + p.wineQuality!) / 2 }))
  .sort((a, b) => b.c - a.c)
  .slice(0, 10)
