...
export async function lookupGoUpc(barcode: string): Promise<GoUpcProduct | null> {
  try {
    const res = await fetch(`/api/goupc?barcode=${encodeURIComponent(barcode)}`, { signal: AbortSignal.timeout(1500) });
...
export async function lookupDrugFacts(barcode: string): Promise<DrugProduct | null> {
  try {
    const res = await fetch(`/api/drugfacts?barcode=${encodeURIComponent(barcode)}`, { signal: AbortSignal.timeout(1500) });
    if (!res.ok) return null;
...
export async function lookupUpcItemDb(barcode: string): Promise<OffProduct | null> {
  try {
    const res = await fetch(`/api/upcitemdb?barcode=${encodeURIComponent(barcode)}`, { signal: AbortSignal.timeout(1500) });
    if (!res.ok) return null;
...
export async function lookupBarcodeLookup(barcode: string): Promise<OffProduct | null> {
  try {
    const res = await fetch(`/api/barcodelookup?barcode=${encodeURIComponent(barcode)}`, { signal: AbortSignal.timeout(1500) });
    if (!res.ok) return null;
    const data: OffProduct | null = await res.json();
    return data?.product_name ? data : null;
  } catch {
...
export async function lookupNihDsld(barcode: string): Promise<NihDsldProduct | null> {
  try {
    const res = await fetch(`/api/nihdsl?barcode=${encodeURIComponent(barcode)}`, { signal: AbortSignal.timeout(1500) });
    if (!res.ok) return null;
