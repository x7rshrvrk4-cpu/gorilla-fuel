...
type EnergyFilter = "all" | "zero-sugar" | "no-aspartame";
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-5 py-12 sm:px-8 sm:py-16">
      ...
      <div className="max-w-2xl">
        <p className="font-display text-sm tracking-[0.3em] text-cyan-400">ENERGY DRINK INTELLIGENCE</p>
        <h1 className="mt-3 font-display text-5xl leading-[0.95] text-foreground sm:text-6xl">
          The <span className="text-cyan-400">Energy Drink</span> Rankings.
        </h1>
        <p className="mt-4 text-muted">
          {ENERGY_PRODUCTS.length} energy drinks scored by the same engine that powers a live scan — sugar,
          calories and additive load per can, with caffeine at a glance. Canadian-market formulas only.
        </p>
      ...
      <div className="mt-4 rounded-sm border border-slate-800 bg-slate-900/40 px-5 py-4">
        <p className="text-xs leading-relaxed text-slate-400">
          ...
          <span className="text-red-300">red &gt;200 mg</span>. Scores use the standard Gorilla engine
          (60% nutrition · 30% additives · 10% organic); energy drinks are NOVA 4 ultra-processed.
        </p>
      </div>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2">
        {products.length > 0 ? (
          ...
          <div className="col-span-full rounded-sm border border-slate-800 bg-slate-900/60 p-8 text-center">
            <p className="text-slate-400">No energy drinks match this filter. Try another combination.</p>
          </div>
        )}
      </div>
